...
def make_parser():
    ...
    join.add_argument('out_file', help="Path to the output file or - for STDOUT")

    extract = subparsers.add_parser('extract', help='Extract masterfile data')
    extract.add_argument('-s', '--skip', metavar='ROWS', type=int, default=0, help="Skip <ROWS> data columns")
    extract.add_argument('--index_column', metavar='COL', help="Use <COL> as the input's index column")
    extract.add_argument('masterfile_path', help="Path to the masterfile to use")
    extract.add_argument('csv_file', help="Path to the CSV input")
    extract.add_argument('out_file', help="Path to the output file or - for STDOUT")

    validate = subparsers.add_parser('validate', help='Validate masterfile')
    validate.add_argument('masterfile_path', help="Path to the masterfile to use")
    validate.add_argument('file', nargs="*", help="Any additional files you wish to check")

    dos = subparsers.add_parser('dos', help=r'Text-to-DOS conversion, using stdin and stdout. ' +
